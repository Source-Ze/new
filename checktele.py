...
async def _(event):
    if ispay2[0] == "yes":
        ...
        @ze.on(events.NewMessage(outgoing=True, pattern=r"\.حالة الصيد"))
        async def _(event):
            if ispay2[0] == "yes":
                if "on" in isclaim:
...
╚════════════════╝
    ...
    {str(eee)}''')
                    if "A wait of" in str(eee):
                        ...
                        await ze.send_message(event.chat.id, " اجاك متاح !")
            else:
                pass
            trys += 1
...
async def _(event):
    if ispay2[0] == "yes":
        ...
        if msg[0] == "تلقائي":  # تثبيت تلقائي عدد يوزر قناة
            ...
            @ze.on(events.NewMessage(outgoing=True, pattern=r"\.حالة التثبيت التلقائي"))
            async def _(event):
                if "on" in isauto:
